...
add_arg('gpus',             str,    '2',                      '训练使用的GPU序号，使用英文逗号,隔开，如：0,1')
add_arg('batch_size',       int,    32,                       '训练的批量大小')
add_arg('num_workers',      int,    4,                        '读取数据的线程数量')
add_arg('num_epoch',        int,    50,                       '训练的轮数')
add_arg('num_classes',      int,    600,                     '分类的类别数量')
...
add_arg('train_data_h5_path',  str,    'pack_data/traindata.h5', '训练数据的数据列表路径')
add_arg('test_data_h5_path',   str,    'pack_data/testdata.h5',  '测试数据的数据列表路径')
add_arg('save_model',       str,    'Models/models_2',                '模型保存的路径')
...
def test(model, metric_fc, test_loader, device):
    ...
    for batch_id, (spec_mag, label) in enumerate(test_loader):
        ...
        label = label.data.cpu().numpy().squeeze(1)
...
def train():
    ...
    # 获取数据
    train_dataset = CustomDataset(args.train_data_h5_path)
    train_loader = DataLoader(dataset=train_dataset,
                              batch_size=args.batch_size * len(device_ids),
                              shuffle=True,
                              num_workers=args.num_workers)
    test_dataset = CustomDataset(args.test_data_h5_path)
